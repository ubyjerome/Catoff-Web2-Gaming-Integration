...
export const prepareConnectWalletTransaction = async (userPublicKey: string): Promise<string> => {
  const connection = new Connection("https://api.devnet.solana.com", "confirmed"); // Use the appropriate RPC URL
...
export const confirmConnectWalletTransaction = async (signedTransaction: string): Promise<string> => {
  const transactionBuffer = Buffer.from(signedTransaction, "base64");
  const tx = await web3Connection.sendRawTransaction(transactionBuffer, { skipPreflight: false, preflightCommitment: 'confirmed' });
  return tx;
};

export const prepareCreateWagerTransaction = async (amount: number, creatorPublicKey: string): Promise<string> => {
  const programId = new PublicKey(Configs.web3.solanaProgramId);
  ...
  const transaction = new Transaction().add(
    ...
    new TransactionInstruction({
      ...
      data: Buffer.from([1, ...new Uint8Array(new Uint32Array([amount]).buffer)]), // Instruction index 1 for create_wager
    })
  );
...
export const prepareJoinWagerTransaction = async (wagerId: string, participantPublicKey: string, amount: number): Promise<string> => {
  const programId = new PublicKey(Configs.web3.solanaProgramId);
  ...
  const transaction = new Transaction().add(
    new TransactionInstruction({
      ...
      data: Buffer.from([2, ...new Uint8Array(new Uint32Array([amount]).buffer)]), // Instruction index 2 for add_participant
    })
  );
